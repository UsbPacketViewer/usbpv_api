...
typedef UPV_HANDLE (UPV_CALL *pfnt_upv_open_device_fast)(
        const char* option,
        int option_len,
        void* context,
        pfn_packet_handler callback);
...
/**
 * open device in fast mode, the ts parameter in callback function always zero
 * the nano parameter represent the tick count in 60MHz in 24 bit
 * @brief upv_open_device_fast
 * @param option
 * @param option_len
 * @param context
 * @param callback
 * @return
 */
UPV_API UPV_HANDLE UPV_CALL upv_open_device_fast(
        const char* option,
        int option_len,
        void* context,
        pfn_packet_handler callback);
